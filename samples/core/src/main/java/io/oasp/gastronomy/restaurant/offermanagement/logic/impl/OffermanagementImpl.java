...
public class OffermanagementImpl extends AbstractComponentFacade implements Offermanagement {
  ...
  @Override
  public List<SpecialEto> findSpecialsByCriteria(SpecialSearchCriteriaTo searchCriteriaTo) {

    List<SpecialEntity> specials = getSpecialDao().findBySearchCriteria(searchCriteriaTo);
  ...
  public SpecialEto saveSpecial(SpecialEto special) {
    ...
    SpecialEntity saved = getSpecialDao().save(getBeanMapper().map(special, SpecialEntity.class));
    return getBeanMapper().map(saved, SpecialEto.class);
  }
